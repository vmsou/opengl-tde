...
// Constructors
Cube::Cube(float size, const v3f& pos, const c4f& c1) : PooledObject3D<Cube>{ pos, c1 }, size{ size } {}
Cube::Cube(const DisplayFile& df) : PooledObject3D<Cube>{ df.pos, df.color } {
	const JSONObject& obj = df.data;
	if (obj.has("size")) this->size = obj.get<float>("size"); else this->size = 50;
}
...
void Cube::addVertices(const v3f* (&face)[Length]) const {
	...
	if (modifiers.grid_enabled) {
		...
		glVertex3f(m1);
		glVertex3f(m1 + (out * (size / 2.0f)));
		glVertex3f(m2);
		glVertex3f(m2 + (out * (size / 2.0f)));
		glEnd();
	}
